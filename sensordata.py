...
class BotDetector:
    def __init__(self):
        ...
        self.timestamps = [
            self.inittime + random.randint(40, 80),
            0,
            random.randint(500, 800),
            random.randint(1000, 1500),
        ]
        self.lasteventtime = self.inittime + self.timestamps[2]
        self.timezonedata = random.choice(
            [
                "Sun Aug 05 1945 19:16:00 GMT-0400 (Eastern Daylight Time)",
        ...
        self.browserdata = {
            "global": {
                "innerHeight": pixeldata[2],
                "innerWidth": pixeldata[0],
                "outerHeight": pixeldata[1],
                "outerWidth": pixeldata[0],
                "screenX": 0,
                "screenY": 0,
            ...
            "screen": {
                "height": (pixeldata[1] + 40),
                "width": pixeldata[0],
                "availHeight": pixeldata[1],
                "availWidth": pixeldata[1],
                "pixelDepth": 24,
                "colorDepth": 24,
            },
            "navigator": {
                ...
                "buildID": None,
                "cpuClass": None,
                "hardwareConcurrency": random.choice([12, 24]),
                "maxTouchPoints": 0,
                "platform": "Win32",
                "product": "Gecko",
                "productSub": "20030107",
        ...
        self.devicedata = {
            "parameters": {
                "antialias": True,
                "maxAnisotropy": 16,
                "dimensions": ["11", "11024", "3276732767"],
                "params": {
                    "alphaBits": 8,
                    "blueBits": 8,
                    "greenBits": 8,
                ...
                "debugInfo": {
                    "vendor": "Google Inc.",
                    "renderer": random.choice(GRAPHICSDRIVER),
                },
            },
            "shaderPrecisions": [
                "23127127",
    ...
    def processrawscript(self, x):
        ...
        self.scriptdata = {
            "length": len(funcstr),
            "whitespace": len(re.findall("\s", funcstr)),
            "punctuators": len(re.findall("[.{([,;=/]", funcstr)),
        }
        ...
        while i < len(k):
            ...
            l = [
                ord(k[s]) << 8 | ord(k[s + 1]) for s in range(max(0, j - 100), j + 100)
            ]
        ...
        while len(enck) < 4 and count < 200:
            ...
            if s == prfunc:
                ...
                p = (j + 4, g)
                j, g = ord(k[j]) << 16 | ord(k[j + 1]) << 8 | ord(k[j + 2]), ord(
                    k[j + 3]
                )
            elif s == afunc:
            ...
            if vdbn[s] >= 3 and "if" in vdb[s]:
                ...
                if l:
                    if vdbn[s] == 3:
                        ...
                        j, g = ord(k[j]) << 16 | ord(k[j + 1]) << 8 | ord(
                            k[j + 2]
                        ), ord(k[j + 3])
                else:
                    j += vdbn[s]
    ...
    def encode(self, data):
        ...
        data = encrypt(
            ...
            iv1,
            iv2,
            self.encryptionseed1,
            self.encryptionseed2,
        ...
        return base64encode(
            struct.pack(">I", iv1) + struct.pack(">I", iv2) + data, self.alphabet
        )
    ...
    def decode(self, data):
        ...
        data = encrypt(
            self.encryptionkey,
            [ord(data[i]) for i in range(8, len(data))],
            iv1,
            iv2,
            self.encryptionseed1,
            self.encryptionseed2,
    ...
    def generateevents(self):
        ...
        while (x - tl) > 1000:
            ...
            while k > 0:
                ...
                tr = tl - self.inittime
                self.mousemoveevents["recent"].append(
                    {"eventType": 4, "timestamp": tr, "x": startx, "y": starty}
                )
                if (
                    len(self.mousemoveevents["throttled"]) == 0
                    or (tr - self.mousemoveevents["throttled"][-1]["timestamp"])
                    > self.throttledmousemoveeventtimeout
                ):
            ...
            if (x - tl) < 1000 or random.randint(0, 2) == 1:
                self.mousebuttonevents.append(
                    {
                        "button": 0,
                        "eventType": 1,
                        "sequenceNumber": self.mousesequencenumber,
                        "target": {
                            "id": "",
                            "name": "",
                            "nodeType": 1,
        ...
        self.mousemoveevents["recent"] = self.mousemoveevents["recent"][
            -self.recentmousemoveeventslimit :
        ]
        self.mousemoveevents["throttled"] = self.mousemoveevents["throttled"][
            -self.throttledmousemoveeventslimit :
        ]
    ...
    def generatedata(self):
        ...
        )  # plugin data for chrome
        signals.append(
            [
                "11",
                {
                    "dataFragment": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAASwA",
                    "hashcode": 1037736760,
                },
            ]
        )  # canvas fingerprint, text 'Hel$&?6%){mZ+#@'
        signals.append(["12", False])  # chrome.webstore
        signals.append(["13", False])  # checking window.Image
        signals.append(
            ["14", simplehash(self.consoleproperties)]
        )  # checking properties of the object window.console
        signals.append(
            [
                "15",
                {
                    "hasToSource": False,
                    "sourceHash": 0,
                    "stringHash": simplehash(
                        "function createElement() { [native code] }"
                    ),
                },
            ]
        )  # hash of document.createElement.toString()
        ...
        signals.append(
            [
                "42",
                {
                    "hash": simplehash("\x00".join(self.mathproperties)),
                    "mathProperties": self.mathproperties,
                },
            ]
        )
        ...
        mmoves["throttled"] = [
            p
            for p in mmoves["throttled"]
            if p["timestamp"] < mmoves["recent"][0]["timestamp"]
        ]
        signals.append(
            [
                "47",
                {
                    "mouseButtonEvents": self.mousebuttonevents,
                    "mouseMoveEvents": mmoves,
                },
            ]
        )
        signals.append(
        ...
        signals.append(
            ["61", self.animationframetimes]
        )  # 10 consecutive requestAnimationFrame
        signals.append(
            [
                "97",
                {
                    "afterReady": {"InstallTrigger": True, "controllers": True},
        ...
        signals.append(
            [
                "111",
                {
                    ...
                    "x-mpegurl": "",
                    "mp4": "maybe",
                    "mp4; codecs=mp4a.40.2": "probably",
                    "ogg; codecs=opus": "probably",
                    "ogg; codecs=speex": "",
                    "webm; codecs=vorbis": "probably",
                    'wav; codecs="0"': "",
                    'wav; codecs="1"': "probably",
                    'wav; codecs="2"': "",
                    "wave": "",
                    "wave; codecs=0": "",
                    "wave; codecs=1": "",
                    ...
                    'webm; codecs="vorbis,vp9"': "probably",
                    'mp4; codecs="avc1.42c00d"': "probably",
        ...
        signals.append(
            ["m", mhash(2166136261, "199" + "3w5e11264sg00" + self.uuidtoken[7:20])]
        )  # Object.keys(window).length=199, parseInt('0xFFFFFFFFFFFFFBFF').toString(36)="3w5e11264sg00", 784054202+1382082059=2166136261
        signals.append(["r", "199"])
        data = Encoder().encode(
            {"errors": [], "signals": signals},
            [
                ["timestamp"],
                [
                    "channelCount",
                ...
                ["argumentsValue", "hasArguments", "hasGlobal", "hasProcess"],
                ["nonce"],
                [
                    "avgAlpha",
                    "avgBeta",
                ...
                ["hash"],
                ["description", "message", "name", "num", "stack", "stacktrace"],
    ...
    def getheaders(self):
        ...
        x[self.headernameprefix + INTEGRITY_KEY] = base36(
            simplehash(self.uuidtoken + m)
        )
